...
export function UpdateCharacter({ character, onCancel }: Props) {
  ...
  const { mutate, isPending } = client.character.update.useMutation({
    onSuccess: () => {
      showToast("Personnage modifié avec succès", 2000, "success");
      void queryClient.invalidateQueries({ queryKey: ["character.getAll"] });
      if (onCancel) onCancel();
    },
    onError: () => {
      showToast("Erreur lors de la modification du personnage", 2000, "error");
    },
  });

  function submit(data: UpdateCharacterDto) {
  ...
  return (
    <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 20 }}>
      ...
      <TouchableOpacity style={[styles.button, { backgroundColor: "#6B7280" }]} onPress={onCancel}>
        <Text style={styles.buttonText}>Annuler</Text>
      </TouchableOpacity>

      <TouchableOpacity style={styles.button} onPress={() => void form.handleSubmit(submit)()}>
